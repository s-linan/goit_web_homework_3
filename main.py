...
def delete_source_folder(path: Path) -> None:
    print(f"Можна видаляти {path}")
    inp = input(f"Видалити {path}? y/yes - так, n/no - ні")
    if inp == 'y' or inp == 'yes':
        try:
            path.rmdir()
            print(f"{path} видалено.")
        except OSError:
            print(f"Can't delete folder: {path}")
    elif inp == 'n' or inp == 'no':
        print(f"{path} залишилась.")
    else:
